...
**クリップボード画像の貼り付け**
クリップボードにある画像（Win+Shift+S で撮ったスクリーンショットなど）を、Ctrl+V でエディタに直接貼り付けられます。画像はRedmine標準の添付の仕組みでアップロードされ、対応する記法がカーソル位置に挿入されます（Markdownなら `![](ファイル名)`、Textileなら `!{width: ...}.ファイル名!`）。この機能は添付フォームがある画面（チケットの説明・注記、Wiki など）でのみ動作します。添付できない画面（プロジェクトの説明、ウェルカムメッセージ等）では、アップロードできないのに記法だけが入る不整合を避けるため、意図的に無効化しています。なお、クリップボードの読み取りはネイティブの paste イベント経由で行うため、右クリックの「貼り付け」メニューと違ってクリップボード読み取り権限を必要とせず、自己署名証明書の環境でも動作します。

**画像記法のホバーでサムネイル表示**
本文中の画像記法にマウスカーソルを乗せると、その添付ファイルのサムネイル画像とファイル名・日付がツールチップで表示されます。添付の挿入画面で見えるものと同じサムネイルです。
...
**編集中の差分マーカー**
編集を始めると、エディタを開いた時点の内容と現在の内容を比べて、変更箇所が左端の余白に色付きで表示されます。追加した行は緑の縦線、変更した行は青の縦線、削除した位置には小さな赤い三角マーカーが出ます。マーカーをクリックすると変更前のテキストがインラインパネルで開き、↶ ボタンを押すとその変更だけを元に戻せます（戻した直後でも Ctrl+Z で取り消し可能）。差分計算は自前実装の git 互換 Myers + xdl_change_compact を使っており、`git diff` と同じ結果に VS Code 流のペアリングを足した形になります。

**説明欄の変更履歴と差分表示（チケットのみ）**
チケットの説明欄について、ツールバーの時計アイコンから過去版を閲覧できます。ドロップダウンには説明を変更した各注記が `#注記番号　著者　日付` の形で並び、項目ごとに「→ 次の版」と「→ 現在」の2通りが選べます。選ぶとエディタが左右の差分表示に切り替わり（左=変更前、右=変更後、両方読み取り専用）、行ごとに赤背景／緑背景で着色され、行頭に `-`／`+` のマーカーが付きます。左右のスクロールは対応位置で同期します。差分表示を閉じるには **編集** ボタン、右上の `×`、または **ESC** キーが使えます。編集中の本文はそのまま保持されます。

**カーソル行のインラインblame表示（チケットのみ）**
チケットの説明欄を編集している間、カーソルがある行の末尾に「`著者　・　相対日付　・　#注記番号`」がうっすらと表示されます。表示の根拠は「その行の文字列が最初に登場した注記の著者」で、VS CodeのGit blameと同じ考え方です。自分が今書いたばかりの行（履歴上にまだ存在しない行）には何も表示されません。データは変更履歴と同じスナップショットを使うため、追加のサーバ通信なしで動作します。

**blameのホバー詳細カード**
カーソル行のblame表示にマウスを1秒以上ホバーすると、すぐ脇に詳細カードがポップアップします。著者名、相対日付と絶対日付、注記番号バッジ、そして説明欄の `+N行 / -M行` の増減サマリを表示します。カード内の **差分を見る** ボタンをクリックすると、その注記の差分表示（前の版↔この版）が直接開きます。カードの上にマウスを乗せている間は閉じないので、そのままボタンを押せます。説明欄を変更していない注記についてはカード自体が表示されません。
...
├── init.rb                          # プラグイン登録 + ViewHook + アセットのsymlink生成
├── app/
│   └── controllers/
...
├── public_dist/                    # 素パス配信するアセット（init.rbが public/monaco_assets へ自動シンボリックリンク）
│   ├── vs/                          # Monaco本体
...
起動時に、プラグインの `init.rb` が自動でシンボリックリンクを張り、同梱アセットを素パスで配信できるようにします。

```
<REDMINE_ROOT>/public/monaco_assets -> <REDMINE_ROOT>/plugins/redmine_monaco_editor/public_dist
```

つまり**手動コピーは不要です。** これにより `vs/`（Monaco本体）と `textgrid/`（表ビルダーライブラリ）の両方が `/monaco_assets/` 配下で参照できるようになります。リンクは `public/monaco_assets` が存在しない場合のみ作成されるため、再起動を繰り返しても問題ありません。

### 動作確認

ブラウザでチケットやwikiの編集画面を開き、エディタがMonacoに変わっていれば成功です。`<host>/monaco_assets/vs/loader.js` に直接アクセスして200が返ることでも確認できます。

エディタが出ない場合は、手動でシンボリックリンクを張ってください。
...
それでも出ない場合は、Webサーバがsymlinkを辿れる設定かを確認してください（nginxの `disable_symlinks` 等が有効になっていないか）。

## アンインストール

1. `plugins/redmine_monaco_editor/` を削除
2. `public/monaco_assets`（シンボリックリンク）を削除
3. Redmine再起動
